	...
	//内
	 var fzdMapServer = "/proxy/arcgis/rest/services/prm_map/MapServer";
	//外
	 var serverREST = "http://202.96.18.90:8064/arcgis/rest/services";
 	//
	...
	//地井
	var djUrl = fzdMapServer +"/6";
	//电杆
	var dgUrl = fzdMapServer +"/4";
	//管段
	var gdUrl = fzdMapServer +"/7";
	//杆路段
	var gldUrl = fzdMapServer +"/5";
	//局所
	var jsUrl = fzdMapServer +"/2";
	//端局局界
	var djjUrl = fzdMapServer +"/8";
	//区局局界
	var qjjUrl = fzdMapServer +"/9";
	//
	var lyFields = ["BUILDINGNAME","BUILDINGNO"];
	var pfFields = ["NAME","FLDTAG","BUNGALOWNO"];
	var fzdFields = ["NAME","REFID"];
	var djFields = ["CODE","REFID"];
	var gdFields = ["A_B_CODE","REFID"];
	var dgFields = ["CODE","REFID"];
	var gldFields = ["CODE","REFID"];
	var jsFields = ["NAME","REFID"];
...
//定义地图图层的ID		
	...
	function initializeEsriJS(){
		require([
			"dojo/parser",
			"esri/Color",
			"esri/geometry/Point",'esri/geometry/Circle',"esri/tasks/IdentifyTask","esri/tasks/IdentifyParameters", "dojo/_base/lang",
			"esri/graphic","esri/graphicsUtils","esri/dijit/Scalebar",
			...
			function (parser, Color,Point,Circle,IdentifyTask,IdentifyParameters,lang,Graphic,graphicsUtils,Scalebar,
	...
	//添加地图加载时点击地图的监听事件
			...
			connect.connect(map, "onLoad", function(evt) {
				...
				// var user = JSON.parse(localStorage.getItem("map_Extent"))
				// if(user){
				// 	console.log(user)
				// 	map.setExtent(new Extent(user))
				// 	localStorage.removeItem('map_Extent')
				// }
				function searchTheperiphery(onOff){
					localStorage.setItem("onOff",onOff)
				}
				gisParams.searchTheperiphery = searchTheperiphery;
				map.on("click",function(evt){
					window.focus()
					clearTempGraphics();
					//点击地图任意坐标生成红色十字
					var onOffs = localStorage.getItem("onOff")
					// map.graphics.clear();
					if(onOffs == 'true'){
						var point = new Point(evt.mapPoint.x,evt.mapPoint.y,map.spatialReference);
						gisParams.rangeSearch(point);
						//map.centerAt(point);
			...
			function rangeSearchs(pt,scope,resourcetype){ 
					var lyids = [];
					if(resourcetype==='Well'){
						lyids.push(6);
					}else if(resourcetype==='Pole'){
						lyids.push(4);
					}else if(resourcetype==='Pipe'){
						lyids.push(7);
					}else if(resourcetype==='Ganluduan'){
						lyids.push(5);
					}else if(resourcetype==='Build'||resourcetype==='ODF'||resourcetype==='Cable'){
						lyids.push(0);
					}else if(resourcetype==='OutdoorPoint'){
						lyids.push(3);
					}else if(resourcetype==='Ziyuanyupan'){
						lyids = [0,3];
					}
    			map.graphics.clear();
    			var geom = new Circle(pt, { "radius": scope });
				  var tmpName = 'fanwei';
    			...
    			yuan = new Graphic(geom,sym);
    			yuan.setAttributes({"SEL_LYNAME":"buffer_circle"});
    			map.graphics.add(yuan);
				var identifyParams = new IdentifyParameters();
				var _identifyTask = new IdentifyTask(fzdMapServer);
				identifyParams.layerIds = lyids;
               ...
               _identifyTask.execute(identifyParams,lang.hitch(this,function(results){
            	   if(results.length > 0){
            		    var retinfos = [];
            		    for(var i=0; i < results.length;i++){
							var item = results[i];
							var graphic = item.feature;
							var id = graphic.attributes["REFID"];
							var lyid = item.layerId;
							var devType 
							if(lyid == 0){
								devType = 'JZW'
							}else if(lyid == 3){
							...
							var node = {
								"DEVTYPE":resourcetype,
								"REFID":id,
								"TYPES":devType
							};
							retinfos.push(node);
						}
						gisParams.resourcegather(retinfos);
					}else{
						gisParams.showError("未查询到资源");
					}
				}),function(err){     
					gisParams.showError("查询失败!");       	   
				});        	  
			}
			gisParams.rangeSearchs = rangeSearchs;

			//地图当前页面中心点
			// map.on("pan-end",function(evt){
			// 	localStorage.setItem("map_Extent",JSON.stringify(evt.extent))
			// 	console.log(JSON.stringify(evt.extent))	
			// });
			...
			// function tests(str1,str2){
			// 	alert('经度：' + str1 + "纬度：" + str2)
			// }
			// gisParams.tests = tests;
	...
	//添加放置点图层fzd layer
			//控制放置点图层显隐
			function fzdControls(fzdControl){
				fzdSymbol = getMarkerSybmol("placepoint");
				if(fzdLayer === undefined){
					fzdLayer = new FeatureLayer(fzdUrl,
						{mode: FeatureLayer.MODE_ONDEMAND,id:LayerIDConstant.FZD,outFields:fzdFields});
				}	
				...
				fzdLayer.setVisibility(fzdControl);
			...
			gisParams.fzdControls = fzdControls;
			...
			// connect.connect(fzdLayer, "onUpdate", hideLoading); 
	//添加地井图层dj layer
			function djControls(djControl){
				if(djLayer === undefined){
					djLayer = new FeatureLayer(djUrl,
						{mode: FeatureLayer.MODE_ONDEMAND,id:LayerIDConstant.DJ,outFields:djFields});
				}	
				djLayer.setSelectionSymbol(getMarkerSybmol("selpoint_blue"));	
				djLayer.setMinScale(lyMinScale);
				djLayer.setVisibility(djControl);
				map.addLayer(djLayer);
				connect.connect(djLayer, "onUpdate", hideLoading); 

			//点击地井层触发的方法
				connect.connect(djLayer, 'onClick', function(evt) {
					if(mapOprate.curTool == "add" || mapOprate.curTool == "selectpoint"){
						return;
					}
					gisParams.showManholeResult(evt.graphic.attributes["REFID"]);
					clearTempGraphics();
					clickFeatureEvent(djLayer,evt);
					cancelEvent(evt);
				});
				
			}
			gisParams.djControls = djControls;
	//添加管段图层gd layer
			function gdControls(gdControl){
				if(gdLayer === undefined){
					gdLayer = new FeatureLayer(gdUrl,
						{mode: FeatureLayer.MODE_ONDEMAND,id:LayerIDConstant.GD,outFields:gdFields});
				}	
				gdLayer.setSelectionSymbol(getMarkerSybmol("tubulation"));
				gdLayer.setMinScale(lyMinScale);
				gdLayer.setVisibility(gdControl);
				map.addLayer(gdLayer);
				connect.connect(gdLayer, "onUpdate", hideLoading); 

			//点击管段层触发的方法
				connect.connect(gdLayer, 'onClick', function(evt) {
					if(mapOprate.curTool == "add" || mapOprate.curTool == "selectpoint"){
						return;
					}
					gisParams.showTubulationResult(evt.graphic.attributes["REFID"]);
					clearTempGraphics();
					clickFeatureEvent(gdLayer,evt);
					cancelEvent(evt);
				});
			}
			gisParams.gdControls = gdControls;
	//添加电杆图层dg layer
			function dgControls(dgControl){
				if(dgLayer === undefined){
					dgLayer = new FeatureLayer(dgUrl,
						{mode: FeatureLayer.MODE_ONDEMAND,id:LayerIDConstant.DG,outFields:dgFields});
				}	
				dgLayer.setSelectionSymbol(getMarkerSybmol("selpoint_blue"));
				dgLayer.setMinScale(lyMinScale);
				dgLayer.setVisibility(dgControl);
				map.addLayer(dgLayer);
				connect.connect(dgLayer, "onUpdate", hideLoading); 

			//点击电杆层触发的方法
				connect.connect(dgLayer, 'onClick', function(evt) {
					if(mapOprate.curTool == "add" || mapOprate.curTool == "selectpoint"){
						return;
					}
					gisParams.showPoleResult(evt.graphic.attributes["REFID"]);
					clearTempGraphics();
					clickFeatureEvent(dgLayer,evt);
					cancelEvent(evt);
				});
			}
			gisParams.dgControls = dgControls;
	//添加杆路段图层gld layer
			function gldControls(gldControl){
				if(gldLayer === undefined){
					gldLayer = new FeatureLayer(gldUrl,
						{mode: FeatureLayer.MODE_ONDEMAND,id:LayerIDConstant.GLD,outFields:gldFields});
				}	
				gldLayer.setSelectionSymbol(getMarkerSybmol("tubulation"));
				gldLayer.setMinScale(lyMinScale);
				gldLayer.setVisibility(gldControl);
				map.addLayer(gldLayer);
				connect.connect(gldLayer, "onUpdate", hideLoading); 

			//点击杆路段层触发的方法
				connect.connect(gldLayer, 'onClick', function(evt) {
					if(mapOprate.curTool == "add" || mapOprate.curTool == "selectpoint"){
						return;
					}
					gisParams.showBarsectionResult(evt.graphic.attributes["REFID"]);
					clearTempGraphics();
					clickFeatureEvent(gldLayer,evt);
					cancelEvent(evt);
				});
			}
			gisParams.gldControls = gldControls;
	//添加局所图层js layer
			function jsControls(jsControl){
				if(jsLayer === undefined){
					jsLayer = new FeatureLayer(jsUrl,
					{mode: FeatureLayer.MODE_ONDEMAND,id:LayerIDConstant.JS,outFields:jsFields});
				}	
				jsLayer.setSelectionSymbol(getMarkerSybmol("selpoint_blue"));
				jsLayer.setMinScale(lyMinScale);
				jsLayer.setVisibility(jsControl);
				map.addLayer(jsLayer);
				connect.connect(jsLayer, "onUpdate", hideLoading); 

			//点击局所层触发的方法
				connect.connect(jsLayer, 'onClick', function(evt) {
					if(mapOprate.curTool == "add" || mapOprate.curTool == "selectpoint"){
						return;
					}
					gisParams.showOfficeResult(evt.graphic.attributes["REFID"],evt.graphic.attributes["NAME"]);
					clearTempGraphics();
					clickFeatureEvent(jsLayer,evt);
					cancelEvent(evt);
				});
			}
			gisParams.jsControls = jsControls
	//添加端局局界图层djj layer
			function djjControls(djjControl){
				if(djjLayer === undefined){
					djjLayer = new FeatureLayer(djjUrl,
						{mode: FeatureLayer.MODE_ONDEMAND,id:LayerIDConstant.DJJ});
				}	
				djjLayer.setMinScale(lyMinScale);
				djjLayer.setVisibility(djjControl);
				map.addLayer(djjLayer);
				connect.connect(djjLayer, "onUpdate", hideLoading); 
			}
			gisParams.djjControls = djjControls;
	//添加区局局界图层qjj layer
			function qjjControls(qjjControl){
				if(qjjLayer === undefined){
					qjjLayer = new FeatureLayer(qjjUrl,
						{mode: FeatureLayer.MODE_ONDEMAND,id:LayerIDConstant.QJJ});
				}	
				qjjLayer.setMinScale(lyMinScale);
				qjjLayer.setVisibility(qjjControl);
				map.addLayer(qjjLayer);
				connect.connect(qjjLayer, "onUpdate", hideLoading); 
			}
			gisParams.qjjControls = qjjControls;
//添加建筑物图层 jzwlayer
			var jzwCollection = {
...
//从搜索楼宇点击定位
			function showLocalInMap(name,constructionNo,buildingType){
				// console.log(name,constructionNo,buildingType)
				clearTempGraphics();
				...
				findParams.searchFields = [lyFields[0]];
...
//从搜索平方点击定位
			function showLocalInMap2(name,constructionNo,buildingType){
				...
				findParams.searchFields = [pfFields[0]];
				findParams.searchText = name;
				findTask.execute(findParams, function(results) {
					if(results && results.length > 0){
						for(var i=0;i<results.length;i++){
							var graphic = results[i].feature;
							if(graphic.attributes["BUNGALOWNO"] == constructionNo){
								lySearchGraphic = graphic;
								lySearchGraphic.setSymbol(getMarkerSybmol("selpolygon"));
								lySearchGraphic.draw();
								map.graphics.add(lySearchGraphic);
...
//显示机房和壁挂点
			...
			function getGraphicData(){
				...
				if(sendList.length > 0){
					gisParams.getBuildingItem(sendList,map.__LOD.level,jfClickGraphic);
				}
			}
			//显示机房
			...
			function showCRoomInMap(data){
				...
				while(count > 0){
					...
					if (currentGraphic.geometry.contains(point)) {
						// var graphic = null;
						if(item.type == 249){
							jfClickGraphic = new Graphic(point,sym_jf);
						}else if(item.type == 382){
							jfClickGraphic = new Graphic(point,sym_bgd);
						}
						map.graphics.add(jfClickGraphic);
						pointList.push(jfClickGraphic);
						count--;
					}
				}
			}
			gisParams.showCRoomInMap = showCRoomInMap;
			function cleanRoom(jfOnoff){
				if(jfOnoff == false){
					map.graphics.clear();
				}
			}
			gisParams.cleanRoom = cleanRoom;
			//获取随机数
...
//点击平房层触发的方法		    
			connect.connect(pfLayer, 'onClick', function(evt) {	
				...
				var constructionNo = evt.graphic.attributes["BUNGALOWNO"];
				// console.log("楼宇："+name+"("+constructionNo+")");
...
//----------------------------------------------------------------------------------------------------------
	//搜索地井，电杆，管段，杆路段，局所，光缆定位
	function ziYuanLocation(ids,lyid,guanglanId,type){
	    if(!ids){    
	      return;
	    }
	    //根据不同的资源类型，定位不同的图层QueryTask
	    var queryTask = new QueryTask(fzdMapServer+"/" + lyid);
	    var query = new Query();
	    query.returnGeometry = true;
	    query.outFields = ["*"];
		if(ids instanceof Array){
			query.where = " REFID in  (" + ids.toString() + ")";
		}else{
			query.where = " REFID in  (" + ids + ")";
		}
	//     query.where = " REFID in (" + ids + ")";
	    queryTask.execute(query,function(results){
		LocateGraphicsAt(results.features);
        for(var i=0;i<results.features.length;i++){
		if(lyid == 5 || lyid == 7){
			gySearchGraphic = results.features[i];
			gySearchGraphic.setSymbol(getMarkerSybmol("tubulation"));
			gySearchGraphic.draw();
            map.graphics.add(gySearchGraphic);
//             map.setLevel(8);
//               return true;
			}else{
				gySearchGraphic = results.features[i];
				gySearchGraphic.setSymbol(getMarkerSybmol("selpoint_blue"));
				gySearchGraphic.draw();
	            map.graphics.add(gySearchGraphic);
	            map.setLevel(8);                     
	            // return true;
...
        }  
			if(lyid == 7){
				if(type == 'optical'){
					gisParams.showOpticalCableResult(guanglanId);
				}else if(type == 'tubulation'){
					gisParams.showTubulationResult(ids);
				}else if(type == 'bianhao'){
					gisParams.circuitQuery()
				}
			}else if(lyid == 5){
				gisParams.showBarsectionResult(ids)
			}else if(lyid == 6){
				gisParams.showManholeResult(ids); 
			}else if(lyid == 4){
				gisParams.showPoleResult(ids);
			}else if(lyid == 2){
				gisParams.showOfficeResult(ids);
			} 
      }, function (err) {
         gisParams.showError("暂无位置信息！");
      });
	}
	gisParams.ziYuanLocation = ziYuanLocation;
	function LocateGraphicsAt(gs) {
	  var extent = graphicsUtils.graphicsExtent(gs);
	  if (extent != null) {
	    if(gs.length > 1){
...
//定义定位图标symbol
			function getMarkerSybmol(resType){
				...
				else if(resType == "tubulation"){ //线
					symbol = new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID,
						new Color([7,245,245]), 2);
				}
				else if(resType == "redDottedCircle"){
...
//清除临时图层方法	            //clear  tmp graphic  //map.graphics
			function clearTempGraphics(){
				...
				// if(gdSelGraphic){
				// 	resetSymbol(gdLayer,gdSelGraphic);
				// 	gdSelGraphic.setSymbol(gdSymbol);
				// 	gdSelGraphic.draw();
				// 	gdSelGraphic = null;	
				// }
				if(gySearchGraphic){
					map.graphics.remove(gySearchGraphic);
					gySearchGraphic = null;
				}
				if(djSearchGraphic){
					map.graphics.remove(djSearchGraphic);
					djSearchGraphic = null;
				}
				if(gdSearchGraphic){
					map.graphics.remove(gdSearchGraphic);
					gdSearchGraphic = null;
				}
				if(dgSearchGraphic){
					map.graphics.remove(dgSearchGraphic);
					dgSearchGraphic = null;
				}
				if(gldSearchGraphic){
					map.graphics.remove(gldSearchGraphic);
					gldSearchGraphic = null;
				}
				if(jsSearchGraphic){
					map.graphics.remove(jsSearchGraphic);
					jsSearchGraphic = null;
				}
				if(jzwSelGraphic){
...
//清除选中方法	            //clear selection
			function clearSelection(curLyid){	            	
				array.forEach(map.graphicsLayerIds, function (lyid) {
					...
					// else if(lyid == LayerIDConstant.DJ){
					// 	return;
					// }
					if(curLyid != lyid){
...
//点击各图层时调用的方法	            //click event
			function clickFeatureEvent(layer,evt){
				...
				if("default" == mapOprate.curTool){
					...
					else if(LayerIDConstant.GD == lyid){
						if(gdSelGraphic){
							resetSymbol(gdLayer,gdSelGraphic);
						}
						gdSelGraphic = evt.graphic;
						gdSelGraphic.setSymbol(getMarkerSybmol("tubulation"));
						gdSelGraphic.draw();
					}
					else if(LayerIDConstant.GLD == lyid){
						if(gldSelGraphic){
							resetSymbol(gdLayer,gldSelGraphic);
						}
						gldSelGraphic = evt.graphic;
						gldSelGraphic.setSymbol(getMarkerSybmol("tubulation"));
						gldSelGraphic.draw();
					}
...
//转换几何中心				
			...
			function locatetogeom(graphic,newGeom){
				// console.log(newGeom,newGeom.type)
				// console.log(newGeom.getExtent())
				switch (newGeom.type) {
					case "point":
					...
					case "polygon":
						...
						break;
					case "polyline":
						map.centerAt(newGeom.getExtent().getCenter());
						break;
				}	
			}
		});      
