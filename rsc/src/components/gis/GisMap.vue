  ...
  <div class="map">
    <div id="map-container">
            ...
            <mu-expand-transition>
              <div v-if="typeShow">
                <mu-tabs  :value.sync="active" color="#2196f3" text-color="#2196f3" indicator-color="#2196f3" inverse full-width style="width:100%;">
                    ...
                    <mu-tab v-if="$util.hasRight('pipeLineInfo')" @click='changeSelect'>管线资源</mu-tab>
                </mu-tabs>
                ...
                <mu-list v-if="active === 3">
                  ...
                  <mu-list-item
                    button
                    @click="ElectricCableSearch('ElectricCable')"
                  >
      ...
      />
       ...
       <i @click='layerBtn' v-if="$util.hasRight('pipeLineInfo')" id="tuceng" class='iconfont icon-tuceng1' style="color:red"></i>
          ...
          <mu-container>
              <div class="select-control-group" >
                <mu-flex class="select-control-row my-row" :key="item.id" v-for="item in layerArr">
                  <mu-checkbox :value="item" v-model="checkbox.selectLevel" :label="item" @change="determineTheLayer(item)"></mu-checkbox>
                </mu-flex>
              </div>
          </mu-container>
      ...
      <mu-bottom-sheet :open.sync="openRange" style="width:94%;margin-left:1.5%;padding-left:10px;margin-bottom:15px;">
          <div style="max-height:50vh;overflow: auto;">
           <mu-form :model="form" label-position="left" label-width="100" auto-validate class="serv-b-form">
               <span style="font-size:16px;font-weight:bold;color:#000;padding:8px 0;display:inline-block">选择资源类型:</span><br/>
               <!-- 有权限时显示 -->
                <mu-form-item v-if="$util.hasRight('lookAround')" label="资源" label-position="left" prop="resourcetype">
                  <mu-select  no-data-text="请选择一项" filterable v-model="form.resourcetype" >
                   <mu-option v-for="item in resourcetypes" :key="item.value" :label="item.name" :value="item.value" style="overflow:visible;width:33.33%;"></mu-option>
                    <!-- <mu-option v-for="item in resourcetypes" :key="item" :label="item" :value="item"></mu-option> -->
                  </mu-select>
                </mu-form-item>
                <!-- 无权限时显示 -->
                <mu-form-item v-else label="资源" label-position="left" prop="resourcetype">
                  <mu-select no-data-text="请选择一项" filterable v-model="form.resourcetype" >
                   <mu-option style='width:33.33%;' v-for="item in resourcetypest" :key="item.value" :label="item.name" :value="item.value"></mu-option>
                  </mu-select>
                </mu-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      layerArr:['放置点','地井','管段','电杆','杆路段','局所','端局局界','区局局界','机房和壁挂点'],
      active:0,
      openManhole:false,
      ...
      form:{
        scope:200,
        resourcetype: 'Ziyuanyupan'
      },
      point:{},
      onOff:false,
      jfOnoff:false,
      resourcetypes:[
        {name:'资源预判',value:'Ziyuanyupan'},
        {name:'楼宇',value:'Build'},
        {name:'放置点',value:'OutdoorPoint'},
        {name:'地井',value:'Well'},
        {name:'电杆',value:'Pole'},
        {name:'管段',value:'Pipe'},
        {name:'杆路段',value:'Ganluduan'},
        {name:'光缆',value:'Cable'},
        {name:'ODF',value:'ODF'}
        ],
      resourcetypest:[
  ...
  created(){
    this.checkbox.selectLevel = JSON.parse(localStorage.getItem("aa")) || this.checkbox.selectLevel
  ...
  methods: {
    ...
    closeSelectLayer(){
      ...
      // this.determineTheLayer()
    },
    //确定图层选择
    determineTheLayer(val){
      // this.selectLayer = false;
      localStorage.setItem("aa",JSON.stringify(this.checkbox.selectLevel))
      ...
      this.jfOnoff = this.checkbox.selectLevel.includes('机房和壁挂点')
      if(this.jfOnoff == true){
        if(val == '机房和壁挂点'){
          if(this.jifangList.length > 0){
          window.mapifram.gisParams.showCRoomInMap(this.jifangList);
          }else{
            return;
          }
        }
      }else{
        window.mapifram.gisParams.cleanRoom(this.jfOnoff);
      }
    },
    //选取中心点
    rangeSearch(point){
    ...
    defineSelect(){
      this.openRange = false
      window.mapifram.gisParams.rangeSearchs(this.point,this.form.scope,this.form.resourcetype);
    },
    //搜索结果集合
    resourcegather(retinfos){
      var resourceName,resourceIdList=[],JZWArr = [],FZDArr = [],resourceIdObj={}
      retinfos.map((item)=>{
          resourceName = item.DEVTYPE;
          resourceIdList.push(item.REFID)
          if(resourceName == 'Ziyuanyupan'){
            if(item.TYPES == 'JZW'){
              JZWArr.push(item.REFID)
            }else if(item.TYPES == 'FZD'){
              FZDArr.push(item.REFID)
            }
            resourceIdObj = {'JZW':JZWArr,'FZD':FZDArr}
          }
      })
      // var retinfos_obj = {"resourceName":resourceName,"resourceIds":resourceIdList}
      // var retinfos_json = JSON.stringify(retinfos_obj)
      //参数说明：
      // resourceName：ODf,局向，楼宇，根据楼宇图层返回的楼宇ID进行查询。光缆，根据周边井资源进行查询。
      let params=new URLSearchParams();
      params.append("type",resourceName);
      if(resourceName == 'Ziyuanyupan'){
        params.append("resourceId",JSON.stringify(resourceIdObj));
      }else{
        params.append("resourceId",resourceIdList);
      }
      localStorage.removeItem("datas")
      this.$axios.post("/peripheral/peripheralSearch",params).then((res)=>{
        var resourcetype = res.data.data.resourcetype
         if(res.data.code == 200){
            var data = JSON.stringify(res.data.data.resourcedata)
            localStorage.setItem("datas",data)
              if(resourcetype != 'ziyuanyupan'){
                if(res.data.data.resourcedata.length>0){
                  this.$router.push({path:'/peripheralResources',query:{data:data,type:resourcetype}});
                }
              }else if(resourcetype === 'ziyuanyupan'){
                  if(this.$store.state.authorityContent.includes('resourcePrediction')){
                    this.$router.push({path:'/peripheralResources',query:{data:data,type:resourcetype}});
                  }else{
    ...
    //电缆
    ElectricCableSearch(type){
      this.typeShow = false;
      this.backShow = true;
      this.listShow = true;
      this.flag = false;
      this.$router.push({
        path: "/electricCableList",
        query: {type: type,search: this.mapSearch }
      });
      this.searchVal();
    },
    ...
    success() {
      ...
      window.mapifram.gisParams.resourcegather = this.resourcegather;
      window.mapifram.gisParams.showOpticalCableResult = this.showOpticalCableResult
      window.mapifram.gisParams.circuitQuery = this.circuitQuery
      window.mapifram.gisParams.initializeEsriJS = this.initializeEsriJS
      // var fzdControl = this.checkbox.selectLevel.includes('放置点')
      // window.mapifram.gisParams.fzdControls(fzdControl);
      // var djControl = this.checkbox.selectLevel.includes('地井')
      // window.mapifram.gisParams.djControls(djControl);
    ...
    getBuildingItem(ids) {
      ...
      this.$axios.post("/building/buildingitem", params).then(response => {
        if (response.data.data.length > 0) {
          this.jifangList = response.data.data
