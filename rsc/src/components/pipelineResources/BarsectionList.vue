<template>
  <mu-container   class="BarsectionList">
      ...
      <mu-list toggle-nested >
          ...
          <div v-for="(item,index) in data" :key="index">
            ...
            <mu-list-item  nested :open="open === index">
                ...
                <!-- 定位 -->
                <mu-list-item-action @click="showBarsectionInMap(item.id)" style="margin-right:20px;">
                ...
                <mu-list-item button  slot="nested" style="background:#F5F5F5;">
                    <mu-list-item-title>
                            <mu-row  class="wall_item">
                                <mu-col span="16">杆路段类型|{{item.buildingType}}</mu-col>
                            
                                <mu-col span="6">杆路段类别|{{item.buildingCategory}}</mu-col>
                            </mu-row>
                    </mu-list-item-title>
                </mu-list-item>
...
export default {
    ...
    methods:{
        ...
        loadData(){
            ...
            params.append("ganluduanbianhao",this.search);
        ...
        showBarsectionInMap(id){

            window.mapifram.gisParams.ziYuanLocation(id,5);
