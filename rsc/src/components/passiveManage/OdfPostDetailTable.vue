<template>
  <div class="OdfPostDetailTable">
    ...
    <div class="pasdev-postInfo-explain">
      ...
      <mu-button v-if="$util.hasRight('ODFportStatusUpdate') && (status === '4')" :color="color3" small style=" margin-right: 0px;float:right;background:rgb(128,128,128); color:#fff;" @click="openAlertDialog3()">空闲</mu-button>
    </div>
    <!-- 该状态弹窗1 -->
    ...
    <!-- 该状态弹窗3 -->
    <mu-dialog title="确认修改？" width="600" max-width="80%" :esc-press-close="false" :overlay-close="false" :open.sync="openAlert3">
        <mu-button slot="actions" flat color="primary" @click="change3()">确认</mu-button>
        <mu-button slot="actions" flat color="primary" @click="closeAlertDialog3">取消</mu-button>
    </mu-dialog>      
  </div>
</template>
<script>
